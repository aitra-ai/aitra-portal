  ...
  <div class="min-h-screen bg-gradient-to-br from-blue-50 via-white to-purple-50 flex items-center justify-center px-4">
    ...
    <div class="relative bg-white rounded-2xl shadow-xl w-full max-w-md p-8">
      ...
      <div v-if="!success" class="space-y-3 mb-5">
        <button
          class="w-full flex items-center justify-center gap-3 h-11 rounded-lg border border-gray-200 bg-white hover:bg-gray-50 transition-colors text-sm font-medium text-gray-700"
...
import { sendEmailCode, signupWithEmail, getOAuthUrl } from '../api/casdoor'

const { t } = useI18n()
const router = useRouter()

function oauthLogin(provider: 'github' | 'google') {
